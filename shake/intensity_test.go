...
func TestIntensity(t *testing.T) {
	...
	for _, intensity := range intensities {
		...
		if i := Intensity(Moratalla2020{}, intensity.v); i != intensity.p {
			t.Errorf("invalid Moratalla2020 rawintensity [%g cm/s]: %d (calculated) != %d (expected)", 100.0*intensity.v, i, intensity.p)
		}
	}
}
