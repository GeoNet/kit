...
func TestSQSSendMessage(t *testing.T) {
	...
	// ACTION
	_, err = client.SendMessage(awsCmdQueueURL(), WithBody(testMessage))
...
func TestSQSSendMessageWithAttributes(t *testing.T) {
	...
	_, err = client.SendMessage(
		...
		WithAttributes(map[string]string{testMessageAttributeKey: testMessageAttributeValue}))
...
func TestSQSSendMessageWithDelay(t *testing.T) {
	// ARRANGE
	setup()
	defer teardown()
	...
	// ACTION
	_, err = client.SendMessage(awsCmdQueueURL(), WithBody(testMessage), WithDelay(5)) // delay seconds
	assert.Nil(t, err)                                                                 // fail fast here to avoid loop queue check
