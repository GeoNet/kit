...
func (d *DLConn) SetId(program, username string) error {
	...
	dlp := DLPacket{
		Header: []byte(fmt.Sprintf("%s %s", dlCmdId, id)),
	}
...
func (d *DLConn) WriteMS(data []byte) error {
	...
	dlp := DLPacket{
		Header: []byte(fmt.Sprintf("%s %s/MSEED %v %v A %v",
			dlCmdWrite, msr.SrcName(false),
			hpTime(msr.StartTime()),
			hpTime(msr.EndTime()),
			len(data)),
		),
		Body: data,
...
func (d *DLConn) sendPacket(dlp DLPacket) (*DLPacket, error) {

	if err := d.setDeadline(); err != nil {
		return nil, err
	}

	out, err := dlPacketToBytes(dlp)
	if err != nil {
		return nil, err
	}
